#include "PhysicsTools/NanoAODTools/interface/CEffUser2D.hh"
#include <assert.h>
#include <iomanip>
...
//--------------------------------------------------------------------------------------------------
void CEffUser2D::printHist2DLatex(const TH2D *hEff, const TH2D *hErrl,
                                  const TH2D *hErrh, ostream &os) {
  ...
  for (Int_t iy = 0; iy <= ny; iy++) {
    for (Int_t ix = 0; ix <= nx; ix++) {
      if (ix == 0 && iy == 0) {
        ...
        os << "& $" << hEff->GetYaxis()->GetBinLowEdge(iy) << "<p_{T}<"
           << hEff->GetYaxis()->GetBinLowEdge(iy + 1) << "$ ";
      } else if (iy == 0) {
        os << "& $" << hEff->GetXaxis()->GetBinLowEdge(ix) << "<\\eta<"
           << hEff->GetXaxis()->GetBinLowEdge(ix + 1) << "$ ";
      } else {
        ios_base::fmtflags flags = os.flags();
        os.precision(3);
        os << "& $" << fixed << hEff->GetBinContent(hEff->GetBin(ix, iy))
           << " \\pm " << hErrl->GetBinContent(hErrl->GetBin(ix, iy)) << "$ ";
        os.flags(flags);
      }
    }
    os << " \\\\" << endl;
  ...
  for (Int_t iy = 0; iy <= ny; iy++) {
    for (Int_t ix = 0; ix <= nx; ix++) {
      if (ix == 0 && iy == 0) {
        ...
        os << "[" << setw(4) << h->GetYaxis()->GetBinLowEdge(iy) << ","
           << setw(4) << h->GetYaxis()->GetBinLowEdge(iy + 1) << "]";
      } else if (iy == 0) {
        os << "[" << setw(4) << h->GetXaxis()->GetBinLowEdge(ix) << ","
           << setw(4) << h->GetXaxis()->GetBinLowEdge(ix + 1) << "]";
      } else {
        ios_base::fmtflags flags = os.flags();
        os.precision(7);
...
//--------------------------------------------------------------------------------------------------
Float_t CEffUser2D::getValue(const TH2D *h, const Double_t x,
                             const Double_t y) {
  Int_t ix = 0;
  Int_t iy = 0;
  const Int_t nx = h->GetNbinsX();
  ...
  for (Int_t i = 1; i <= nx; i++) {
    if ((x >= h->GetXaxis()->GetBinLowEdge(i)) &&
  ...
  for (Int_t i = 1; i <= ny; i++) {
    if ((y >= h->GetYaxis()->GetBinLowEdge(i)) &&
