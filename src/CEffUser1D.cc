#include "PhysicsTools/NanoAODTools/interface/CEffUser1D.hh"
#include <assert.h>
#include <iomanip>
...
//--------------------------------------------------------------------------------------------------
void CEffUser1D::loadEff(TGraphAsymmErrors *gr) { graph = gr; }
...
void CEffUser1D::print(const Double_t *yval, std::ostream &os) {
  ...
  for (Int_t i = 0; i < graph->GetN(); i++) {
    os << "[" << setw(4) << xval[i] - xerrl[i] << "," << setw(4)
       << xval[i] + xerrh[i] << "]";
  }
  os << endl;
  for (Int_t i = 0; i < graph->GetN(); i++) {
...
Int_t CEffUser1D::getBin(const Double_t x) {
  ...
  const Double_t *xerrh = graph->GetEXhigh();
  for (Int_t i = 0; i < graph->GetN(); i++) {
    if ((x >= (xval[i] - xerrl[i])) && (x < (xval[i] + xerrh[i])))
      return i;
  }
  return -1;
}
