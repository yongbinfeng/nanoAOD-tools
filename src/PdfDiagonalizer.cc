#include "PhysicsTools/NanoAODTools/interface/PdfDiagonalizer.h"

PdfDiagonalizer::PdfDiagonalizer(){};

PdfDiagonalizer::PdfDiagonalizer(const char *name, RooWorkspace *w,
                                 RooFitResult &result)
    : // eigen_gl(new TVectorD()),
      name_(name), parameters_(result.floatParsFinal()) {
  ...
  for (int i = 0; i < n; ++i) {
    snprintf(buff, sizeof(buff), "%s_eig%d[-5,5]", name,
             i); // this is not actually a Gaussian??! -> Just creates a
                 // variable with given range, value=0
  ...
  // each is equal to the transpose matrx times the square root of the
  // eigenvalue (so that we get unit gaussians)
  for (int i = 0; i < n; ++i) {
    RooArgList coeffs;
    for (int j = 0; j < n; ++j) {
      snprintf(buff, sizeof(buff), "%s_eigCoeff_%d_%d[%g]", name, i, j,
               vectors(i, j) * sqrt(values(j)));
      coeffs.add(*w->factory(buff));
    }
    snprintf(buff, sizeof(buff), "%s_eigBase_%d[%g]", name, i,
             (dynamic_cast<RooAbsReal *>(parameters_.at(i)))->getVal());
    coeffs.add(*w->factory(buff));
...
RooAbsPdf *PdfDiagonalizer::diagonalizeWithEigenVariations(RooAbsPdf &pdf,
                                                           RooFitResult &result,
                                                           int index_eig,
                                                           int nSigma) {
  //
  ...
  // Check that the fit status is reasonable and that the covariance matrix is
  // OK
  ...
  if (ret->getVal() <= 0 || ret->getVal() > 1 || std::isnan(ret->getVal()) ||
      !std::isfinite(ret->getVal())) {
    return &pdf;
  }
  return ret;
}
