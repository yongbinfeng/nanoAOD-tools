...
#include "PhysicsTools/NanoAODTools/src/CEffUser1D.cc"
#include "PhysicsTools/NanoAODTools/src/CEffUser2D.cc"
...
public:
  ...
  vector<double> getUncSta(TLorentzVector *l1, int q1,
                           TLorentzVector *l2 = nullptr, int q2 = 0) {
    return computeAllUncertainties(sta, u_sta, l1, q1, l2, q2);
  }

  vector<double> getUncSel(TLorentzVector *l1, int q1,
                           TLorentzVector *l2 = nullptr, int q2 = 0) {
    return computeAllUncertainties(sel, u_sel, l1, q1, l2, q2);
  }

  // return a vector with each of the uncertainties propagated to final acc
  // values
  vector<double> computeAllUncertainties(basicEff &effs, effUnc &uncs,
                                         TLorentzVector *l1, int q1,
                                         TLorentzVector *l2 = nullptr,
                                         int q2 = 0) {
    vector<double> res;
    res.push_back(
        computeWithUnc(effs, uncs.fsrPos, uncs.fsrNeg, l1, q1, l2, q2));

    res.push_back(computeWithUnc(effs, uncs.mcPos, uncs.mcNeg, l1, q1, l2, q2));
    res.push_back(
        computeWithUnc(effs, uncs.bkgPos, uncs.bkgNeg, l1, q1, l2, q2));
    res.push_back(
        computeWithUnc(effs, uncs.tagPos, uncs.tagNeg, l1, q1, l2, q2));

    return res;
  }

  double computeWithUnc(basicEff &effs, CEffUser2D &varPos, CEffUser2D &varNeg,
                        TLorentzVector *l1, int q1,
                        TLorentzVector *l2 = nullptr, int q2 = 0) {
    double effdata = 1, effmc = 1;
    if (q1 > 0) {
      effmc *= effs.mcPos.getEff(l1->Eta(), l1->Pt());
      effdata *= effs.dataPos.getEff(l1->Eta(), l1->Pt()) *
                 varPos.getEff(l1->Eta(), l1->Pt());
    } else {
      effmc *= effs.mcNeg.getEff(l1->Eta(), l1->Pt());
      effdata *= effs.dataNeg.getEff(l1->Eta(), l1->Pt()) *
                 varNeg.getEff(l1->Eta(), l1->Pt());
    }
    if (!l2) {
    ...
    if (q2 > 0) {
      ...
      effdata *= effs.dataPos.getEff(l2->Eta(), l2->Pt()) *
                 varPos.getEff(l2->Eta(), l2->Pt());
    } else {
      effmc *= effs.mcNeg.getEff(l2->Eta(), l2->Pt());
      effdata *= effs.dataNeg.getEff(l2->Eta(), l2->Pt()) *
                 varNeg.getEff(l2->Eta(), l2->Pt());
    }
    return effdata / effmc;
  }

  // compute efficiencies
  double fullEfficiencies(TLorentzVector *l1, int q1,
                          TLorentzVector *l2 = nullptr, int q2 = 0) {
  ...
  double dataOnly(TLorentzVector *l1, int q1, TLorentzVector *l2 = nullptr,
                  int q2 = 0) {
    double corr = 1;
  ...
  double statHLTdilep(TH2D *pos, TH2D *neg, double wgt, TLorentzVector *l1,
                      int q1, TLorentzVector *l2 = nullptr, int q2 = 0) {
  ...
  double statUncHLT(TLorentzVector *l1, int q1, TH2D *pos, TH2D *neg,
                    double wgt) {
    return statUnc(hlt, l1, q1, pos, neg, wgt);
  }
  double statUncSel(TLorentzVector *l1, int q1, TH2D *pos, TH2D *neg,
                    double wgt) {
    return statUnc(sel, l1, q1, pos, neg, wgt);
  }
  double statUncSta(TLorentzVector *l1, int q1, TH2D *pos, TH2D *neg,
                    double wgt) {
  ...
  void loadEfficiencyFile(basicEff &effs, TString dir, TString q1, TString q2) {
    ...
    effs.dataPos.loadEff((TH2D *)dp->Get("hEffEtaPt"),
                         (TH2D *)dp->Get("hErrlEtaPt"),
                         (TH2D *)dp->Get("hErrhEtaPt"));

    TFile *dn = new TFile(dataNegName);
    effs.dataNeg.loadEff((TH2D *)dn->Get("hEffEtaPt"),
                         (TH2D *)dn->Get("hErrlEtaPt"),
                         (TH2D *)dn->Get("hErrhEtaPt"));

    TFile *mp = new TFile(mcPosName);
    effs.mcPos.loadEff((TH2D *)mp->Get("hEffEtaPt"),
                       (TH2D *)mp->Get("hErrlEtaPt"),
                       (TH2D *)mp->Get("hErrhEtaPt"));

    TFile *mn = new TFile(mcNegName);
    effs.mcNeg.loadEff((TH2D *)mn->Get("hEffEtaPt"),
                       (TH2D *)mn->Get("hErrlEtaPt"),
                       (TH2D *)mn->Get("hErrhEtaPt"));
  ...
  void loadUncertaintyFile(effUnc &uncs, TString filename) {
    ...
    uncs.fsrNeg.loadEff((TH2D *)fSys->Get("hFSRNeg"), (TH2D *)fSys->Get(""),
                        (TH2D *)fSys->Get(""));
    uncs.fsrPos.loadEff((TH2D *)fSys->Get("hFSRPos"), (TH2D *)fSys->Get(""),
                        (TH2D *)fSys->Get(""));

    uncs.mcNeg.loadEff((TH2D *)fSys->Get("hMCNeg"), (TH2D *)fSys->Get(""),
                       (TH2D *)fSys->Get(""));
    uncs.mcPos.loadEff((TH2D *)fSys->Get("hMCPos"), (TH2D *)fSys->Get(""),
                       (TH2D *)fSys->Get(""));

    uncs.bkgNeg.loadEff((TH2D *)fSys->Get("hBkgNeg"), (TH2D *)fSys->Get(""),
                        (TH2D *)fSys->Get(""));
    uncs.bkgPos.loadEff((TH2D *)fSys->Get("hBkgPos"), (TH2D *)fSys->Get(""),
                        (TH2D *)fSys->Get(""));

    uncs.tagNeg.loadEff((TH2D *)fSys->Get("hTagNeg"), (TH2D *)fSys->Get(""),
                        (TH2D *)fSys->Get(""));
    uncs.tagPos.loadEff((TH2D *)fSys->Get("hTagPos"), (TH2D *)fSys->Get(""),
                        (TH2D *)fSys->Get(""));
  ...
  double statUnc(basicEff eff, TLorentzVector *l1, int q1, TH2D *pos, TH2D *neg,
                 double wgt) {
    double var = 0.0;
    if (q1 > 0) {
      Double_t effdata = eff.dataPos.getEff(l1->Eta(), l1->Pt());
      Double_t errdata =
          TMath::Max(eff.dataPos.getErrLow(l1->Eta(), l1->Pt()),
                     eff.dataPos.getErrHigh(l1->Eta(), l1->Pt()));
      Double_t effmc = eff.mcPos.getEff(l1->Eta(), l1->Pt());
      Double_t errmc = TMath::Max(eff.mcPos.getErrLow(l1->Eta(), l1->Pt()),
                                  eff.mcPos.getErrHigh(l1->Eta(), l1->Pt()));
      Double_t errSta =
          (effdata / effmc) * sqrt(errdata * errdata / effdata / effdata +
                                   errmc * errmc / effmc / effmc);
      pos->Fill(l1->Eta(), l1->Pt(), errSta * wgt);
      var += errSta * errSta;
    } else {
      Double_t effdata = eff.dataNeg.getEff(l1->Eta(), l1->Pt());
      Double_t errdata =
          TMath::Max(eff.dataNeg.getErrLow(l1->Eta(), l1->Pt()),
                     eff.dataNeg.getErrHigh(l1->Eta(), l1->Pt()));
      Double_t effmc = eff.mcNeg.getEff(l1->Eta(), l1->Pt());
      Double_t errmc = TMath::Max(eff.mcNeg.getErrLow(l1->Eta(), l1->Pt()),
                                  eff.mcNeg.getErrHigh(l1->Eta(), l1->Pt()));
      Double_t errSta =
          (effdata / effmc) * sqrt(errdata * errdata / effdata / effdata +
                                   errmc * errmc / effmc / effmc);
  ...
  double statUncHLTDilep(TLorentzVector *lep1, int q1,
                         TLorentzVector *lep2 = nullptr, int q2 = 0) {
    ...
    Double_t errdata = TMath::Max(hlt.dataPos.getErrLow(l1->Eta(), l1->Pt()),
                                  hlt.dataPos.getErrHigh(l1->Eta(), l1->Pt()));
    Double_t effmc = hlt.mcPos.getEff(l1->Eta(), l1->Pt());
    Double_t errmc = TMath::Max(hlt.mcPos.getErrLow(l1->Eta(), l1->Pt()),
                                hlt.mcPos.getErrHigh(l1->Eta(), l1->Pt()));
    ...
    errdata = TMath::Max(hlt.dataNeg.getErrLow(l2->Eta(), l2->Pt()),
                         hlt.dataNeg.getErrHigh(l2->Eta(), l2->Pt()));
    effmc = hlt.mcNeg.getEff(l2->Eta(), l2->Pt());
    errmc = TMath::Max(hlt.mcNeg.getErrLow(l2->Eta(), l2->Pt()),
                       hlt.mcNeg.getErrHigh(l2->Eta(), l2->Pt()));
  ...
  double computeHLTSF(TLorentzVector *l1, int q1, TLorentzVector *l2 = nullptr,
                      int q2 = 0) {
    double effdata = 1.0;
    double effmc = 1.0;
    double corr = 1.0;
  ...
  double computeStaSF(TLorentzVector *l1, int q1, TLorentzVector *l2 = nullptr,
                      int q2 = 0) {
    double effdata = 1.0;
    double effmc = 1.0;
    double corr = 1.0;
  ...
  double computeSelSF(TLorentzVector *l1, int q1, TLorentzVector *l2 = nullptr,
                      int q2 = 0) {
    double effdata = 1.0;
    double effmc = 1.0;
    double corr = 1.0;
  ...
  double effHLTData(TLorentzVector *l1, int q1, TLorentzVector *l2 = nullptr,
                    int q2 = 0) {
    double effdata = 1.0;
    double effmc = 1.0;
    double corr = 1.0;
  ...
  double effStaData(TLorentzVector *l1, int q1, TLorentzVector *l2 = nullptr,
                    int q2 = 0) {
    double effdata = 1.0;
    double effmc = 1.0;
    double corr = 1.0;
  ...
  double effSelData(TLorentzVector *l1, int q1, TLorentzVector *l2 = nullptr,
                    int q2 = 0) {
    double effdata = 1.0;
    double effmc = 1.0;
    double corr = 1.0;
